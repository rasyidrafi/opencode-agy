...
describe("opt-in official Antigravity ACP live checks", () => {
  test.skipIf(!live)("initializes, streams, resumes, and accepts an image prompt", async () => {
    ...
    const worker = await createAcpWorker({ cwd: process.cwd(), executable: detection.executable, executableArgs: detection.args, model: "gemini-3.7-flash-high", permissionPolicy: "allow-always", stallTimeoutMs: 120_000 });
    ...
    try {
      ...
      const resumed = await createAcpWorker({ cwd: process.cwd(), executable: detection.executable, executableArgs: detection.args, model: "gemini-3.7-flash-high", sessionId, permissionPolicy: "allow-always", stallTimeoutMs: 120_000 });
      try {
